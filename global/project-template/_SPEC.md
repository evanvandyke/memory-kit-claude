...
- FOLLOW-UP is a `###` heading inside UP NEXT -- not `**FOLLOW-UP**` bold text, not a `##` peer heading, not a numbered slot. Its annotation (`*(third-party asks only -- pinned, never rises)*`) and its italic description line must match the template verbatim.
...
- The AGENDA header includes two structural blockquotes -- a formatting preservation note and management rules (completion is deletion, demotion not deletion, default is hold). Both are load-bearing guidance that prevents agents from mismanaging the agenda. Repair restores them from the template when missing.
