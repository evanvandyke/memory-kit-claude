...
## Invariants: what "correct" means and what repair checks
...
- Contains the session-end line (`**At session end:** run /wrap.`). Without it, the project silently drops out of the every-5th-session repair clock.
...
- ACTIVE holds at most one item. One item = one deliverable focus; independent workstreams joined by "and" are multiple items. UP NEXT holds at most three items *total*. FOLLOW-UP is **inside** that three, not on top of it: when work is blocked on a third party, one of the three slots **is** the FOLLOW-UP block (2 work slots + 1 FOLLOW-UP when blocked; 3 work slots when nothing's blocked). This is intentional: a deliberate visibility bottleneck that keeps blocked/third-party work in view, not overhead to optimize away.
...
- Formatting is structural, not cosmetic. Tier headers (`## ACTIVE  *(limit 1)*`, etc.), their parenthetical annotations, and the italic description lines beneath them are load-bearing: they tell agents how to use each section. Match means verbatim against the template; paraphrases, truncations, and additions are all drift. When missing, repair restores them from the template. When mangled (wrong wording, wrong heading level), repair corrects to match.
- No emojis in any section or subsection header within the agenda (`## ACTIVE`, `### FOLLOW-UP`, etc.). Repair strips emojis and restores the template header verbatim.
- FOLLOW-UP is a `###` heading inside UP NEXT: not `**FOLLOW-UP**` bold text, not a `##` peer heading, not a numbered slot. Its annotation (`*(third-party asks + calendar gates, pinned, never rises)*`) and its italic description line must match the template verbatim.
- UP NEXT uses numbered slots (`1.` `2.` `3.`) for work items only. Three numbered slots are always present, reflecting the limit: empty slots retain their number as a bare line. Unordered bullets (`-`) or missing slot numbers are drift; repair converts to numbered.
- Every FOLLOW-UP item carries a nudge date. Format: `who` `·` `last sent ask (date + ref)` `·` `nudge [date]`. Default nudge = 3 business days from last sent. Calendar-gated items use: `calendar` `·` `gate: [date]`. Items enter FOLLOW-UP when the ask is sent, close only when the recipient confirms, and the nudge date resets on every outbound touch. Items with metadata in non-canonical formats (prose dates, `who:` labels, free-text nudge notes) are reformatted to the canonical format during repair.
- Same-communication items nest under one parent. When multiple FOLLOW-UP topics came from one communication (e.g. one email covering three questions), they share a single parent entry carrying the who + sent date + nudge date, with topics as indented bullets. This prevents flat lists where items all share the same contact event.
- The AGENDA header includes two structural blockquotes: a formatting preservation note and management rules (completion is deletion, demotion not deletion, default is hold, outbound asks enter FOLLOW-UP). Both are load-bearing guidance that prevents agents from mismanaging the agenda. Repair restores them from the template when missing.

> **On the duplication repair will see:** the tier limits above are authoritative here. The `AGENDA.md` template restates them as a teaching copy, and the route-table is taught in several places by design (the `MEMORY.md` header, DOC-SYSTEM-MAP Layer 3). That overlap is intended, not drift. `_SPEC` is the tiebreaker if any restatement ever disagrees.
...
- **Purge on overflow:** a 16th entry forces dropping one to stay ≤15. This is surfaced, never silent; priority graduated-to-global → superseded → least-applicable. Repair **proposes** purges in `REPAIR-REVIEW.md`; in-session, surface and confirm. Never an auto-rewrite or auto-delete.
- **Durability:** every entry must still earn its slot: graduated-to-global, superseded, and least-applicable entries are cut candidates even below the cap. Repair proposes these cuts; it never auto-edits `feedback.md`.
