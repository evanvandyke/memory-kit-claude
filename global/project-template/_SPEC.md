...
- Every FOLLOW-UP item carries a nudge date. Format: `who` `·` `last sent ask (date + ref)` `·` `nudge [date]`. Default nudge = 3 business days from last sent. Calendar-gated items use: `calendar` `·` `gate: [date]`. Items with metadata in non-canonical formats (prose dates, `who:` labels, free-text nudge notes) are reformatted to the canonical format during repair.
